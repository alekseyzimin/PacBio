...
awk 'BEGIN{p=1;}{if($1 ~/^>/){if(substr($1,2)==$2) p=0; else p=1;} if(p==1) print $0;}' asm_to_asm.delta| delta-filter -r -o 20 /dev/stdin|show-coords -lcHr /dev/stdin | awk '{if($12<$13) print $0}' |merge_matches_and_tile_coords_file.pl 100000 | awk '{if($10>95 && $15>90) print $(NF-1)}' > $ASM_FASTA.duplicates.txt

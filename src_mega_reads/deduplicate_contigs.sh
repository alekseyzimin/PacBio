...
show-coords -lcHr $ASM_DIR/sasm_to_sasm.noself.fdelta | awk '{if($12>$13) print $0}' |merge_matches_and_tile_coords_file_new.pl $MERGE_LEN 2>&1 | perl -ane '{$cov{$F[-1]}+=$F[15];}END{foreach $k(keys %cov){print $k,"\n" if($cov{$k}>60);}}' > $ASM_DIR/sduplicates.txt && \

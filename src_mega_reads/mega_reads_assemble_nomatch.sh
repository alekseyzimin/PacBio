...
create_mega_reads -s $JF_SIZE -m $MER --psa-min 14  --stretch-cap 10000 -k $KMER -u $KUNITIGS -t $NUM_THREADS -B $B --max-count 3000 -d $d  -r $SUPERREADS  -p $PACBIO -o $COORDS.txt.tmp && mv $COORDS.txt.tmp $COORDS.txt
touch .rerun
fi
...
create_mega_reads --stretch-cap 6000 -s $JF_SIZE --psa-min 14 -m 17 -k $KMER -u $KUNITIGS -t $NUM_THREADS -B 13 --max-count 500 -d $d  -r $COORDS.all_mr.fa  -p $PACBIO -o $COORDS.mr.txt.tmp && mv $COORDS.mr.txt.tmp $COORDS.mr.txt
touch .rerun
fi

...
jf_aligner --compact -s $JF_SIZE -m $MER -t  $NUM_THREADS -f -B $B --stretch-cap 10000 --max-count 5000 --psa-min 13 --coords /dev/stdout -u $KUNITIGS -k $KMER -H -r $SUPERREADS -p $PACBIO1  | longest_path -t $NUM_THREADS  -u $KUNITIGS  -k $KMER -d $d /dev/stdin  -o $COORDS.txt.tmp && mv $COORDS.txt.tmp $COORDS.txt || error_exit "mega-reads pass 1 failed";
touch .rerun
fi
...
jf_aligner --compact -s $JF_SIZE -m $(($MER+2)) -t  $NUM_THREADS -f -B $(($B-4)) --stretch-cap 6000 --max-count 2000 --psa-min 13 --coords /dev/stdout -u $KUNITIGS -k $KMER -H -r $COORDS.all_mr.maximal.fa -p <(ufasta extract -v -f $COORDS.single.txt $PACBIO1)  | longest_path -t $NUM_THREADS  -u $KUNITIGS  -k $KMER -d $d /dev/stdin  -o $COORDS.mr.txt.tmp && mv $COORDS.mr.txt.tmp $COORDS.mr.txt || error_exit "mega-reads pass 2 failed";
touch .rerun
fi

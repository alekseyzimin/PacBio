...
if [ ! -s $COORDS.1.fa ] || [ -e .rerun ];then
    ...
    echo "$CA_PATH/blasr to_blasr.\$1.fa   ref.\$1.fa  -nproc 16 -bestn 10 -m 5 2>blasr.err | sort -k6 -S5% | $CA_PATH/pbdagcon -j 8 -t 0 -c 1 /dev/stdin  2>pbdagcon.err | $MYPATH/nucmer -p join.\$1 --maxmatch -l 17 -c 51 -L 200 -t 16 to_join.\$1.fa /dev/stdin && touch consensus.\$1.success" >> do_consensus.sh && \

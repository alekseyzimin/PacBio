...
if [ ! -s $COORDS.1.fa ] || [ -e .rerun ];then
    ...
    echo "$CA_PATH/blasr to_blasr.\$1.fa   ref.\$1.fa  -nproc 16 -bestn 10 -m 5 2>blasr.err | sort -k6 -S5% | $CA_PATH/pbdagcon -j 8 -t 0 -c 1 /dev/stdin  2>pbdagcon.err |tee join_consensus.\$1.fasta | $MYPATH/nucmer --delta /dev/stdout --maxmatch -l 17 -c 51 -L 200 -t 16 to_join.\$1.fa /dev/stdin | $MYPATH/filter_delta_file_for_qrys.pl qrys.txt | $MYPATH/show-coords -lcHq -I 88 /dev/stdin | $MYPATH/extract_merges_mega-reads.pl join_consensus.\$1.fasta > merges.\$1.txt && touch consensus.\$1.success" >> do_consensus.sh && \

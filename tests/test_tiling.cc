...
TEST(Tiling,Uniform) {
  ...
  for(auto& n : nodes) {
    ...
    n.ldensity = density_dist(rng);
    n.lpath = n.ldensity * (n.imp_e - n.imp_s);
    n.lstart = -1;
  }
  ...
  const auto tile_greedy = graph.tile_greedy(sort_greedy, nodes);
  const auto tile_maximal = graph.tile_maximal(sort_maximal, nodes);

...
#here we look for the best extension for current mega-read, must overlap and have the highest score=amount of extension+k-mer coverage, only consider overlapping extensions
	...
	for(my $i=$mega_reads_last_index[$mri];$i<=$#sr;$i++){
	    ...
	    $overlap=overlap_ext($mega_reads_last_sr[$mri],$sr[$i]);#number of extra k-unitigs
	    if($overlap>0){
		@tf=split(/\s+/,@kmers[$i]);
		$ext_score=0;
		for(my $k=$#tf;$k>$#tf-2*$overlap;$k-=2){
		$ext_score+=$tf[$k]-$tf[$k-1];

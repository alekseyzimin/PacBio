...
if(not($f[0] eq $ctg)){
  ...
  #proceed with fixing
    for(my $i=$#fixes;$i>-1;$i--){#going in reverse order to avoid shifting sequence due to indels
      ...
      die("sequence does not match the original $original_seq $originals[$i]") if(($original_seq =~ /a|c|g|t|n|A|C|G|T|N/) && not(uc($originals[$i]) eq uc($original_seq)));
      #then substitute
...
if($ff[3]<=1 && $ff[5]>=$min_alt_obs || $ff[5]/$sum_alleles>0.65){
  push(@fixes,$f[4]);
  push(@originals,$f[3]);
  push(@offsets,$f[1]);
}

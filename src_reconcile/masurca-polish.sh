...
(
  ...
  echo '  $FREEBAYES -C 2 -m 20 -q 20 -R 0 -p 1 -F 0.4 -b <($SAMTOOLS view -h ../$BASM.alignSorted.bam `head -n 1 $1.listnames` 2>>$1.samtools.err |$SAMTOOLS view -S -b /dev/stdin 2>>$1.samtools.err)  -v $1.vcf -f ../$ASM && touch $1.vc.success' >> commands.sh 
  echo 'fi' >> commands.sh

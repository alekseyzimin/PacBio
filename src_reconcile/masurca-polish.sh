...
do
    ...
    case $key in
        ...
        -v|--verbose)
            ...
            ;;
        -h|--help|-u|--usage)
            echo "Usage:  evaluate_consensus_error_rate.sh -a <assembly contigs or scaffolds> -r <'Illumina_reads_fastq1 Illumina_reads_fastq2'> -t <number of threads> [-n] <optional:do not fix errors that are found>"
...
(
  ...
  echo "  $FREEBAYES -C 3 -q 20 -R 0 -p 1 -F 0.2 -E 0 -b <($SAMTOOLS view -h ../$BASM.alignSorted.bam \`head -n 1 \$1.listnames\` 2>>\$1.samtools.err |$SAMTOOLS view -S -b /dev/stdin 2>>\$1.samtools.err)  -v \$1.vcf -f ../$ASM && touch \$1.vc.success" >> commands.sh 
  echo 'fi' >> commands.sh

...
(
  ...
  echo "  $SAMTOOLS view -h ../$BASM.alignSorted.bam \`head -n 1 \$1.listnames\` 2>>\$1.samtools.err |$SAMTOOLS view -Sbu /dev/stdin 2>>\$1.samtools.err | $FREEBAYES -m 0 --min-coverage 3 -R 0 -p 1 -F 0.2 -E 0 -c -v \$1.vcf -f $ASMPATH/$BASM && touch \$1.vc.success" >> commands.sh 
  echo 'fi' >> commands.sh

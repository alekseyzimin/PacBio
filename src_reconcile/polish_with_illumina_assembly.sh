...
do
    ...
    case $key in
        ...
        -v|--verbose)
            ...
            ;;
        -h|--help|-u|--usage)
            echo "Usage: polish_with_illumina_assembly.sh -r <sequence to be polished> -q <sequence to polish with> -t <number of threads> -m <merge polishing sequence alignments slack (advanced)> "
            exit 0
            ;;
        *)
...
nucmer -t $NUM_THREADS -p  $DELTAFILE -l 31 -c 100 $REF $QRY && touch polish_align.success && rm -f filter.success || exit
fi

#delta-filter
if [ ! -e polish_filter.success ];then
parallel_delta-filter.sh $DELTAFILE '-i 97 -1 -l 100  -o 0 ' 9 && mv $DELTAFILE.fdelta $DELTAFILE.1.delta && \
...
if [ $MERGE_SLACK -gt 0 ];then
show-coords -lcHr -I 98 -L 100 $DELTAFILE.1.delta |  reconcile_consensus.pl $REFN.$QRYN.all.fa $QRY > $REFN.$QRYN.all.polished.fa && \
touch polish_replace_consensus.success && rm -f polish_self_map.polish.success || exit
else
show-coords -lcHr -I 98 -L 100 $DELTAFILE.1.delta | merge_matches_and_tile_coords_file.pl $MERGE_SLACK | reconcile_consensus.pl $REFN.$QRYN.all.fa $QRY > $REFN.$QRYN.all.polished.fa && \

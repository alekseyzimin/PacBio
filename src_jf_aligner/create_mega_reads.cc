...
int main(int argc, char *argv[])
{
  ...
  if(args.unitigs_lengths_given) {
    if(!args.k_mer_given)
      create_mega_reads_cmdline::error() << "The mer length used for generating the k-unitigs (-k, --k-mer) is required if the unitig lengths switch (-l, --unitig-lengths) is passed.";
  ...
  align_data.unitigs_lengths(unitigs_lengths.get(), args.k_mer_arg);

...
void ContigGapEst(struct hashtable *ht1,struct hashtable *merge_ht1, struct hashtable2 *ht2, struct hashtable2 *merge_ht2,int K_size,vector<int> &insert_sz_vt,vector<string>& filenames_vt,vector<bool> &LongLib,struct contigs_info * contigs_info,string ContigFilename,bool ResumePE,int64_t totReads,bool MatePair)
{
	...
	for(size_t ii=0;ii<filenames_vt.size();ii+=2)
	{
		...
		{
		sprintf_s(o_sc_r_n,"Pdist_R_lib_%d.txt",lib_no);
		sprintf_s(o_sc_l_n,"Pdist_L_lib_%d.txt",lib_no);
		sprintf_s(o_sc_pd,"Pdist_lib_%d.txt",lib_no);
		sprintf_s(o_sc_inward,"Pdist_inward_lib_%d.txt",lib_no);
		sprintf_s(o_sc_outward,"Pdist_outward_lib_%d.txt",lib_no);
		sprintf_s(o_sc_log,"Pdist_lib_%d_log.txt",lib_no);
		}
		else
		{
		sprintf_s(o_sc_r_n,"PdistIter_R_lib_%d.txt",lib_no);
		sprintf_s(o_sc_l_n,"PdistIter_L_lib_%d.txt",lib_no);
		sprintf_s(o_sc_pd,"PdistIter_lib_%d.txt",lib_no);
		sprintf_s(o_sc_inward,"PdistIter_inward_lib_%d.txt",lib_no);
		sprintf_s(o_sc_outward,"PdistIter_outward_lib_%d.txt",lib_no);
		sprintf_s(o_sc_log,"PdistIter_lib_%d_log.txt",lib_no);
		}
...
void ResolvingRepeatsPE(vector<int> &insert_sz_vt,vector<string>& filenames_vt,struct contigs_info * contigs_info,string ContigFilename,int LinkCovTh,int UniqueLenTh,int ExpCov)
{
	...
	for(int i=1;i<=lib_cnt;++i)
	{
		...
		if(1)//Iter_Scaffold==0)
		{
			sprintf_s(sc_name,"Pdist_L_lib_%d.txt",i);
		}
		else
		{
			sprintf_s(sc_name,"PdistIter_L_lib_%d.txt",i);
		}
		...
		if(MatePair==0)
		{
		sprintf_s(sc_name,"Pdist_R_lib_%d.txt",i);
		}
		else
		{
			sprintf_s(sc_name,"PdistIter_R_lib_%d.txt",i);
		}

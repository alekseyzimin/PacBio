...
struct kmer_info
{
	...
	//uint16_t cov1;
	uint16_t cov1:8,used:1,split_left:1,split_right:1,removed:1,flip:1,marked:1,repeat:1,masked:1;
	uint64_t cod:32,read_idx:32;
...
struct read_t
{
	...
	int read_idx;
	uint64_t read_bits[100000];//bit representation
...
struct ref_t
{
	struct ref_read_t ref_seq;
	map<uint64_t,int> FragEnds,N_Pos;
	vector< map<uint64_t,bool> > repeat_maps;
	uint64_t repeat_cnt;
	int MaxMatch;
	size_t * SuffixArray;
	size_t * MarkerPositions;
	//uint32_t * LocalSuffix;

	uint64_t * LocalSuffix;
...
struct qry_ret
{
	bool align,repeat,match_kmers_only;
	int tot_kmers;
	int matching_kmers;
	map<int64_t,int> match_cnt;
	vector<string> matched_seq;
	vector <int64_t> match_pos;
	vector <int64_t> match_read;
	vector < vector <int64_t> > ordered_read_pos;
	
	vector<string> aligned_seq;
	map<int,vector<int> > match_read_cod;
	map<int,vector<int> > match_ref_read_beg;
	map<int,vector<int> > match_ref_read_end;
	map<int,int > match_ref_read_first,match_ref_read_last;
	vector<int> insertions,deletions,substitutions;
	map<int,vector<int> > match_stats;
	string ret_seq;
...
struct match_cod
{
	uint64_t cod:60,flip:1;
	struct match_cod *next_match;
};
...
bool get_a_qs_read(ifstream & qs_in, int &len, vector<double> &qs_vt)
{
...
// get sub bit array from a bit array.
 inline void get_sub_arr(uint64_t * bitsarr_in,int bitsarr_len,int64_t begin_pos,int sub_sz,uint64_t * bitsarr_out)
{
...
void Init_Read(string &seq,struct read_t & read)
{
	...
	str2bitsarr(seq.c_str(),read.readLen,read.read_bits,Read_arr_sz);

...
mkdir -p reconcile
(cd reconcile && polish_with_illumina_assembly.sh -r ../$COORDS.1.fa -q ../$CA/9-terminator/genome.ctg.fasta -t $NUM_THREADS -m 10000 1> /dev/null && perl -ane '{
  if($F[0]=~/^>/){
    if(length($seq)>0){
      @f=split(/(N{1,})/,uc($seq)); 
      my $n=1;
...
}'  $COORDS.1.fa.genome.ctg.fasta.all.polished.deduplicated.fa > ../$COORDS.1.contigs.fa) && touch reconcile.success || error_exit "reconcile failed"
rm -f merge.success
fi
